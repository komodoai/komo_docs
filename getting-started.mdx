...
<Steps>
  ...
  <Step title="Create Komodo backends">
    ...
    <Accordion title="AWS">
      ...
      - `docker_repo`: This is an ECR repo called `komodo-overlays` that was created when running `komo compute create`. Log into ECR in the AWS console to get the URI for it (it should look like xxxxxxxx.dkr.ecr.your-region.amazonaws.com/komodo-overlays)
      - `ssh-key`: This is the path to an SSH key that was created when running `komo compute create`. It should be `~/.komo/compute-komodo-ssh-key-my-cluster`, replacing `my-cluster` with whatever name you provided to the `komo compute create` command.
      - `basion-ip-address`: This is the IP address of a small (t2.micro) instance that is created as a proxy server to allow you to ssh into your jobs/machines. You can find this by running `komo compute list`.
