...
<Steps>
  ...
  <Step title="Apply promo code">
    We are currently in a private beta, and so you will need a promo code to use Komodo. If you do not have one, send us an email at hello@komodoai.dev
    to get access (and also a free working session with an engineer to help you get started!).

    ```bash
    komo apply-promo-code <your promo code>
    ```
  </Step>
