...
<Steps>
  ...
  <Step title="Create Komodo backends">
    ...
    <Accordion title="AWS">
      ...
      - **aws_batch_queue**: You can find this by running `komo compute list` and looking in the `queue_arns` section. Enter the **last part** (don't include the prefix) of the queue related to this instance. In this example, that would be `komodo-my-cluster-p4-24xl-queue`
    </Accordion>
